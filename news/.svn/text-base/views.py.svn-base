...
def by_tag(request,tag):
	qs = NewsItem.on_site.filter(tags__contains=tag,pub_date__isnull=False)
...
def by_category(request,category_slug):
	...
	qs = NewsItem.on_site.filter(category=the_category,pub_date__isnull=False)
...
def by_author(request,author_slug):
	...
	qs = NewsItem.on_site.filter(author=the_author,pub_date__isnull=False)
	return object_list(request,qs,template_object_name='item')
