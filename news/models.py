...
class NewsManager(models.Manager):
	...
	qs = self.get_query_set().filter(pub_date__isnull=False)#.order_by('-date')
        qs = qs.filter(pub_date__lte=datetime.now()).order_by('-pub_date')
	if limit:
	    return qs[:limit]
	return qs
...
class NewsItem(models.Model):
	...
	return '/news/%s/%s/%s/%s/' % (self.pub_date.strftime('%Y'), self.pub_date.strftime('%b').lower(), self.pub_date.strftime('%d'), self.slug)
        # return ('news-item', (), {
	...
	try:
	    ...
	    return NewsItem.on_site.filter(pub_date__lt=self.pub_date,pub_date__isnull=False)[0]
	except IndexError, e:
	...
	try:
	    # isnull is to check whether it's published or not - drafts don't have dates, apparently
	    return NewsItem.on_site.filter(pub_date__gt=self.pub_date,pub_date__isnull=False).order_by('pub_date')[0]
	except IndexError, e:
	    # print 'Exception: %s' % e.message
	    return None
...
try:
    ...
    class NewsItemPlugin(CMSPlugin):
        """
        """
        num_to_show = models.PositiveIntegerField(_("Number of NewsItem to show"),
